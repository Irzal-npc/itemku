...
const SwitchAccount = () => {
  const { login, register, resetPassword, user, savedAccounts, removeSavedAccount } = useAuth();
    ...
    <div className="min-h-screen bg-background text-foreground font-poppins">
      <div className="container mx-auto py-8 px-4">
        <div className="flex items-center mb-8">
          ...
          <div>
            <h1 className="text-4xl font-bold text-primary mb-2">ITEMKU</h1>
            <p className="text-muted-foreground">Switch Account</p>
          </div>
        </div>
        ...
        <div className="max-w-md mx-auto">
          <div className="bg-card border border-border rounded-lg shadow-lg p-6 mb-6">
            <div className="text-center mb-4">
              ...
              <p className="font-semibold text-card-foreground">{user?.user_metadata?.full_name || user?.email}</p>
          ...
          <div className="bg-card border border-border rounded-lg shadow-lg p-6">
            <h2 className="text-2xl font-bold mb-6 text-card-foreground text-center">
              {isLogin ? 'Sign In to Another Account' : 'Create New Account'}
            </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button 
                ...
                disabled={isLoading}
              >
                {isLoading ? 'Please wait...' : (isLogin ? 'Switch Account' : 'Create Account')}
              </Button>
            </form>

            <div className="mt-6 text-center">
              <p className="text-sm text-muted-foreground">
                {isLogin ? "Need to create a new account?" : "Want to sign in to an existing account?"}{' '}
                <button 
